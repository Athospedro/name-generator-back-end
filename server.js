   ...
   async deleteItem(_, args) {
    ...
    async generateDomains() {
      ...
      for (const prefix of items.filter(item => item.type === 'prefix')) {
        for (const suffix of items.filter(item => item.type === 'suffix')) {
          ...
          const checkout = `https://cart.hostgator.com.br/?pid=d&sld=${url}&tld=.com&domainCycle=2`
          const avaliable = await isDomainAvailable(`${url}.com.br`)
          domains.push({
            name,
            checkout,
            avaliable
    ...
    async generateDomain(_, args) {
      ...
      for (const extension of extensions) {
        ...
        const avaliable = await isDomainAvailable(`${url}${extension}`)
        domains.push({
          name,
          extension,
          checkout,
